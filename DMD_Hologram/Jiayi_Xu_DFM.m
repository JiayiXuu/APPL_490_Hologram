...
NCycle = 50;                % Run search for ideal hologram for that many cycles
lambda = 1.0E-6;            % wavelength [m]
ps = 2*10.0E-6;               % Pixel Size [m]
propagationdistance = 0.08; % Desired propagation distance 
fA = 0.01 ;                  %Focal length lens first lens [m]
fB = 0.2 ;                  %Focal length lens second lens [m]
aperturesize = 0.01 ;      %Radius of the aperture placed in Fourier space[m]
...
axis image; 
xlabel('x,[mm]');ylabel('y [mm]');
title(['Intensity after mask [AU] log scale']); 
colormap gray;
colorbar;
